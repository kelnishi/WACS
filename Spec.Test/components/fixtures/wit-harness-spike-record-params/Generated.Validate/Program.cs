...
namespace WitHarnessSpike.RecordParams.Generated.Validate
{
    /// <summary>
    /// Exercises records-as-direct-params carrying string + list +
    /// nested record fields — the generalized record lower path
    /// flattens each field recursively.
    /// </summary>
    internal static class Program
    {
        public static int Main(string[] args)
        {
            try
            {
                ...
                {
                    Namespace = "WitHarnessSpike.RecordParams.Generated",
                });

                var harnessType = asm.GetType("WitHarnessSpike.RecordParams.Generated.PackagerHarness")!;
                var parcelType = asm.GetType("WitHarnessSpike.RecordParams.Generated.Parcel")!;
                var dimsType = asm.GetType("WitHarnessSpike.RecordParams.Generated.Dimensions")!;
                ...
                // Build a Parcel { name="alpha", tags=["a","bb","ccc"], size={width=12,height=34} }
                var dims = Activator.CreateInstance(dimsType, (uint)12, (uint)34)!;
                var parcel = Activator.CreateInstance(parcelType,
                    "alpha", new string[] { "a", "bb", "ccc" }, dims)!;

                var describe = harnessType.GetMethod("Describe", BindingFlags.Public | BindingFlags.Instance)!;
                var result = (string)describe.Invoke(harness, new object?[] { parcel })!;
                Console.WriteLine($"describe(...) = '{result}'");
                if (result != "alpha (12x34) tags=a,bb,ccc")
                {
                    Console.Error.WriteLine($"FAIL: expected 'alpha (12x34) tags=a,bb,ccc', got '{result}'");
                    return 1;
                }

                Console.WriteLine("PASS — record-of-{string, list<string>, nested record} param lower round-trip green.");
                return 0;
            }
            catch (Exception ex)
            {
        ...
        {
            var sideBySide = Path.Combine(AppContext.BaseDirectory, "packager.component.wasm");
            if (File.Exists(sideBySide)) return sideBySide;
            var devTree = Path.GetFullPath(Path.Combine(
                AppContext.BaseDirectory, "..", "..", "..", "..", "wasm", "packager.component.wasm"));
            if (File.Exists(devTree)) return devTree;
            throw new FileNotFoundException(
                $"packager.component.wasm not found (looked at {sideBySide} and {devTree})");
        }

        private static void BindWasiStubs(WasmRuntime runtime)
        {
            Action<ExecContext, int> drop = (_, _) =>
                throw new NotSupportedException("Packager harness does not implement WASI runtime.");
            ...
            Func<ExecContext, int> getHandle = _ => throw new NotSupportedException("Packager harness does not implement WASI runtime.");
