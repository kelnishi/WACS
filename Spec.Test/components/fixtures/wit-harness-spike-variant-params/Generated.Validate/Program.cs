...
namespace WitHarnessSpike.VariantParams.Generated.Validate
{
    /// <summary>
    /// Exercises variant as direct PARAM. Three cases: two with no
    /// payload (silence, ping) and one with a string payload
    /// (message). The non-message cases zero-pad the joined slot
    /// shape's trailing (ptr, len) positions.
    /// </summary>
    internal static class Program
    {
        public static int Main(string[] args)
        {
            try
            {
                ...
                {
                    Namespace = "WitHarnessSpike.VariantParams.Generated",
                });

                var harnessType = asm.GetType("WitHarnessSpike.VariantParams.Generated.RouterHarness")!;
                var signalType = asm.GetType("WitHarnessSpike.VariantParams.Generated.Signal")!;
                var silenceType = signalType.GetNestedType("Silence")!;
                var pingType = signalType.GetNestedType("Ping")!;
                var messageType = signalType.GetNestedType("Message")!;
                ...
                var describe = harnessType.GetMethod("DescribeSignal", BindingFlags.Public | BindingFlags.Instance)!;

                var silence = Activator.CreateInstance(silenceType)!;
                var ping = Activator.CreateInstance(pingType)!;
                var msg = Activator.CreateInstance(messageType, "hi")!;

                var s = (string)describe.Invoke(harness, new object?[] { silence })!;
                var p = (string)describe.Invoke(harness, new object?[] { ping })!;
                var m = (string)describe.Invoke(harness, new object?[] { msg })!;
                Console.WriteLine($"describe-signal(silence) = '{s}'");
                Console.WriteLine($"describe-signal(ping) = '{p}'");
                Console.WriteLine($"describe-signal(message('hi')) = '{m}'");
                if (s != "silence" || p != "ping!" || m != "msg<hi>")
                {
                    Console.Error.WriteLine($"FAIL: variant lower mismatch");
                    return 1;
                }

                Console.WriteLine("PASS — variant direct param lower round-trip green.");
                return 0;
            }
            catch (Exception ex)
            {
