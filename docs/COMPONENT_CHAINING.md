...
| `--wasi-gfx` | adds `WACS.WASI.GFX`<br>`WACS.WASI.GFX.DependencyInjection`<br>`WACS.WASI.GFX.Webgpu`<br>`WACS.WASI.GFX.Silk` | Typed `[WitSource]` interfaces for `wasi:graphics-context` / `wasi:surface` / `wasi:frame-buffer` / `wasi:webgpu`<br>Bundle + impl classes for direct-link emit<br>Webgpu contract + canonical-ABI bindings<br>Silk.NET/SDL + wgpu-native backend (single backend drives all four WITs against one SDL window). Pair with `--windowed` when the guest opens a surface. |
...
<a id="wasi-gfx-chaining"></a>

### wasi-gfx example (windowed compute + render)

End-to-end run of a wasip2 component that opens a window
through `wasi:surface`, claims a graphics context through
`wasi:graphics-context`, and drives the SDL/Metal swap-chain
through `wasi:webgpu` — the Game of Life windowed parity
fixture is the reference shape.

The wasi-gfx family is four sibling WIT packages
(`wasi:graphics-context` / `wasi:surface` / `wasi:frame-buffer`
/ `wasi:webgpu`); `--wasi-gfx` enables all four. One Silk.NET/SDL
+ wgpu-native backend (`Wacs.WASI.GFX.Silk`) drives the entire
family against a single SDL window.

```sh
wacs run game-of-life-windowed.component.wasm \
    --wasip2 \
    --wasi-gfx \
    --windowed \
    --call start
...
- Loads `WACS.WASI.GFX` + `.DependencyInjection` + `.Webgpu`
  + `.Silk` (typed surfaces + impl classes + backend).
- Reserves the calling (main) thread for the SDL event pump
  (AppKit hard-requires window + Metal-view creation on the
  main thread on macOS); runs the wasm guest on a `Task.Run`
  worker. `--windowed` is what triggers this split — without
  it, a guest that opens a surface either fails outright or
  renders to an orphan layer that never appears on screen.
- `--call start` selects the component's exported `start: func()`
  entrypoint. wasi-gfx fixtures export `start` (no leading
  underscore — the cargo-component convention) rather than the
  WASI-cli `_start` the CLI defaults to.
- Surface / frame / pointer / key events fan through
  `ManualResetPollable` so the guest's `wasi:io/poll.poll(...)`
  wakes on each OS event.

The CPU-side (`graphics-context` / `surface` / `frame-buffer`)
and the GPU-side (`webgpu`) share one `ResourceContext` so
pollables minted by `surface.subscribe-frame()` are reachable
from `wasi:io/poll.poll` — and `device.connect-graphics-context`
on the GPU side drills into the CPU-side surface to attach
the wgpu swap-chain to the SDL window's Metal layer.

The composite for the bundle slot
(`WasiPreview2GfxBundle`, in `WACS.WASI.GFX.DependencyInjection`)
is auto-generated by `CompositeBundleGenerator` from the
`[WacsCompositeBundle]` attribute — no hand-maintained property
list, and `HostPackageResolver` finds it through the same
`ResolveBundleProperty` walk used for the wasi-nn composite.

For headless GPU work (compute kernels, render-to-texture without
a window), drop `--windowed` and skip `surface.constructor` in the
guest; the wgpu host runs on the calling thread and never touches
NSView/AppKit. The three headless parity fixtures
(`wasi-webgpu-hello-compute`, `wasi-webgpu-hello-render`,
`wasi-webgpu-game-of-life`) run in CI this way.

Platform notes:
- macOS arm64 — fully verified; the swap-chain claims the
  window's `CAMetalLayer` after `SilkSurface.DropSdlRenderer`
  releases the SDL renderer's hold on it.
- Windows / Linux — the swap-chain path currently throws
  `PlatformNotSupportedException`; headless wgpu works.

See [`docs/WASI_GFX_USAGE.md`](WASI_GFX_USAGE.md) for the full
guide (programmatic embedding, threading contract, worked
examples).
...
- [`Wacs.WASI.NN/Wacs.WASI.NN/README.md`](../Wacs.WASI/Wacs.WASI.NN/Wacs.WASI.NN/README.md)
  ...
  packages.
- [`docs/WASI_GFX_USAGE.md`](WASI_GFX_USAGE.md) — wasi-gfx
  usage guide: CPU + GPU paths, programmatic embedding,
  threading contract, parity fixtures, worked examples.
- [`Wacs.WASI.GFX/Wacs.WASI.GFX/README.md`](../Wacs.WASI/Wacs.WASI.GFX/Wacs.WASI.GFX/README.md)
  — wasi-gfx contract assembly + IBackend SPI.
