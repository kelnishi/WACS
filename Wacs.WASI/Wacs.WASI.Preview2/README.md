...
- **[Wacs.WASI.Preview2.DependencyInjection/](Wacs.WASI.Preview2.DependencyInjection/)** — `Microsoft.Extensions.DependencyInjection` integration: register host implementations via `services.AddWasiPreview2(...)` instead of constructing each binder by hand. Also ships `WasiPreview2RuntimeScope`, the one-shot disposable embedders use to bind a single component against a single runtime — auto-detects the `Wacs.WASI.NN.DependencyInjection` composite, threads `Preopens` mount pairs into `IPreopens`, and eagerly fires every `*Bindings.BindToRuntime` before returning.
